...
class Radar():
    def __init__(self) -> None:
        self.configFileName = 'IWR1843config.cfg'
        self.CLIport = {}
        self.Dataport = {}
    ...
    def serialConfig(self):
        ...
        # Raspberry pi
        #self.CLIport = serial.Serial('/dev/ttyACM0', 115200)
        #self.Dataport = serial.Serial('/dev/ttyACM1', 921600)

        # Windows
        #self.CLIport = serial.Serial('COM8', 115200)
        #self.Dataport = serial.Serial('COM9', 921600)

        # macOS
        self.CLIport = serial.Serial('/dev/tty.usbmodemR21010501', 115200)
        self.Dataport = serial.Serial('/dev/tty.usbmodemR21010504', 921600)
    ...
    def parseConfigFile(self):
        ...
        self.configParameters["rangeResolutionMeters"] = (
        ...
        self.configParameters["dopplerResolutionMps"] = 3e8 / (2 * startFreq * 1e9 * (
            idleTime + rampEndTime) * 1e-6 * self.configParameters["numDopplerBins"] * numTxAnt)
        self.configParameters["maxRange"] = (
            300 * 0.9 * digOutSampleRate)/(2 * freqSlopeConst * 1e3)
        self.configParameters["maxVelocity"] = 3e8 / \
            (4 * startFreq * 1e9 * (idleTime + rampEndTime) * 1e-6 * numTxAnt)

        return self.configParameters

    def readAndParseData18xx(self):
    ...
    def startRadar(self):
        ...
        self.CLIport, self.Dataport = self.serialConfig()
    ...
    def updateData(self):
        ...
        self.dataOk, self.frameNumber, self.detObj = self.readAndParseData18xx()
